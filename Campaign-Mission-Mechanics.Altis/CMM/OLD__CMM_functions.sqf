...
if ((vehicleVarName _player) == "ZeusPlayer") then 
	{
			...
			["RETURN_MKR",return,"LOCAL"] spawn moveMarker;
...
MonitorMarkers = { //Monitor Markers Also does Mission Startup Title
private ["_txtold","_OLDdays","_txt","_days"];
	...
	while {true} do {
			...
			[_days,"TitleDays",true,false,true] call BIS_fnc_MP;
			_OLDdays = _days;
			sleep 60;
			};
		};
...
StripPlayer = {
//		copyToClipboard format ["%1", magazines player]
//		Usage = [cursorTarget,player] spawn StripPlayer;
private ["_crate","_player"];
_crate = _this select 0;
_player = _this select 1;
	//TFAR DISABLE Comment out of 
